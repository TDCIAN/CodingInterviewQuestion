...
문제 16. Java에서 "=="과 equals(...) 메소드의 차이는 무엇이며 객체의 깊은 비교와 얕은 비교의 차이는 무엇입니까?
- 변수의 참조가 메모리의 동일한 객체를 가리킬 때 "==" 연산은 true를 결과로 반환합니다. 이것이 객체의 "얕은 비교"입니다.
- euquals()는 속성값을 비교하도록 사용자가 제공한 클래스의 equals() 메소드를 실행한 결과를 반환합니다.
- equals() 메소드는 "==" 연산자가 제공하는 얕은 비교와 대조적으로 두 객체간의 논리적인 동일성을 체크 함으로써 "깊은 비교"를 제공합니다.
- 만일 사용자가 클래스의 equals 메소드를 제공하지 않을 경우, 참조가 메모리의 같은 객체를 가리키는지 평가하는 Object 클래스에서 상속된 equals() 메소드가 실행됩니다.
- Object 클래스의 equals 메소드는 "==" 연산자와 동일합니다(즉, 얕은 비교).
  
  
문제 17. 직렬화(Serialization)란 무엇입니까? 어떻게 직렬화에서 클래스의 필드를 제외할 수 있으며 비상주 변수(transient variable)란 무엇입니까?
- 직렬화는 객체를 읽고 쓰는 절차입니다. 이것은 객체의 상태를 연속된 바이트로 저장하고 다시 이 바이트들을 조립하여 라이브 객체(live object)로 되돌려 놓는 과정입니다.
- 객체는 java.io.serializable 인터페이스를 구현하여 직렬화 가능하다고 표시됩니다.
- 이렇게 하여 단순히 클래스가 파일과 같은 바이트의 배열로 저장될 수 있는지 확인하는 직렬화 메커니즘을 적용하게 합니다.
- 비상주 변수는 직렬화할 수 없습니다. 객체 직렬화에서 transient로 표시된 필드는 바이트 스트림으로 전달되지 않습니다.
- 예를 들어 파일 핸들(file handle), 데이터베이스 연결, 시스템 스레드 등이 있습니다.
- 이러한 객체들은 로컬에서만 의미를 가집니다. 따라서 직렬화 클래스에서 transient로 표시해야만 합니다.
  
  
문제 18. Java에서 final과 finally, finalize()의 차이는 무엇입니까?
- final: 상수의 선언입니다. final 클래스는 상속될 수 없으며 final 함수는 재정의될 수 없습니다.
- finally: 예외 처리자입니다. finally 블록은 선택적으로 사용할 수 있으며 try 블록에서 어떤 일(System.exit(0) 호출을 제외하고)이 발생하는지에 관계없이 정리할 수 있는 메커니즘을 제공합니다.
- finalize(): 가비지 콜렉션 시에 사용되는 함수입니다. 가비지 콜렉터에 의해 상태 정리할 수 있도록 객체가 제거되기 전에 호출되는 함수입니다. 파일 핸들, 소켓과 같은 비 메모리 리소스를 해지하기 위해 사용해서는 안됩니다. Java에서 이런 리소스는 한정된 수만 가지며 가비지 콜렉션이 finalize() 함수를 통해 비 메모리 리소스를 해지할지 알 수 없기 때문입니다.
  
  
  
문제 19. Java 가비지 콜렉터에 대해 설명하세요.
- Java에서 객체가 생성될 때 마다 힙(heap) 메모리에 들어갑니다.
- Java에서 힙은 가비지 콜렉션이 가능한 힙(garbage collectable heap)이라고 불립니다.
- 가비지 콜렉션은 강제로 할 수 없습니다.
- 가비지 콜렉션은 메모리가 얼마 남지 않은 상황에서 수행됩니다.
- 가비지 콜렉션이 수행될 때 어떤 라이브 객체에 의해서도 참조되지 않는 객체에 할당된 메모리를 제거합니다.
- 가비지 콜렉터는 System.gc()를 호출하여 메모리의 무효 영역을 정리하지만 강제로 수행시킬 수는 없습니다.
- 객체의 생명은 아무도 참조하지 않을 때 의미를 상실합니다.
- 아무도 참조하지 않는다는 것은 더 이상 필요가 없다는 의미이고, 이것을 가비지 콜렉터가 찾아낼 것입니다.
- Java는 자동으로 이런 객체들을 주기적으로 찾아내고 메모리 낭비를 없애기 위해 이러한 객체들로 소비되는 메모리를 해지합니다.
  
  
  
## Chapter 02. INTRODUCTION
### 2.1 알고리즘이란 무엇인가?
  - 주어진 문제를 풀 때 우리가 해야 할 일은 문제를 풀기 위한 단계적 절차를 두는 것입니다.
  - 알고리즘의 공식적인 정의는 "알고리즘은 주어진 문제를 처리하는 단계별 지침"이라고 할 수 있습니다.
  - 우리가 알고리즘의 각 단계를 입증할 필요는 없습니다.

### 2.2 왜 알고리즘 분석인가?
  - 컴퓨터 공학에는 동일한 문제를 풀기 위한 많은 알고리즘이 존재합니다
  - 예를 들어 정렬의 경우 삽입 정렬, 선택 정렬, 퀵 정렬 등의 많은 알고리즘이 있습니다.
  - 알고리즘 분석은 이들 중 어떤 것이 시간과 공간 사용의 효율성이 좋은지 결정할 수 있게 합니다.
  
### 2.3 알고리즘 분석의 목표
  - 알고리즘 분석의 목표는 주로 실행 시간뿐만 아니라 다른 요인(메모리, 개발자의 노력 등)들을 포함해 알고리즘(또는 솔루션)을 비교하는 것입니다.

### 2.4 실행 시간 분석(Running Time Analysis)이란 무엇인가?
  - 실행 시간 분석이란 문제의 크기(입력 크기)가 증가함에 따라 처리 시간이 어떻게 증가하는지를 결정하는 프로세스입니다.
  - 입력 크기란 입력 요소의 수이며 문제의 유형에 따라 입력의 유형은 달라질 수 있습니다.
  - 일반적으로 우리는 다음과 같은 입력 유형을 만납니다.
    - 배열의 크기
    - 다항식 차수
    - 행렬 내 요소들의 수
    - 바이너리로 표현에서 비트의 수
...
### 2.5 어떻게 알고리즘을 비교하는가?
  - 실행 시간? 특정 컴퓨터에서 측정한 실행 시간은 좋은 척도가 되지 못합니다.
  - 실행 구문의 수? 문장의 수는 프로그래밍 언어 뿐만 아니라 프로그래머의 스타일에 따라 달라지기 때문에 좋은 척도가 되지 못합니다.
  - 이상적인 해결책? 주어진 알고리즘의 실행 시간이 입력 크기 n의 함수(예, f(n))로 표현되고 이들 함수에 상응하는 서로 다른 실행 시간을 비교한다고 가정해 봅시다. 이런 유형의 비교는 기계 시간(machine time) 또는 프로그램 스타일 같은 것들 과는 관계가 없습니다.
  
  
### 2.6 성장률(Rate of Growth)이란 무엇인가?
  - 입력 함수에 따라 실행 시간이 증가하는 비율을 성장률이라고 합니다.

### 2.7 일반적으로 사용되는 성장률

### 2.8 분석 유형
  - 주어진 알고리즘을 분석하기 위해서는 어떤 입력에서 알고리즘에서 적은 시간이 걸리고 어떤 입력에서 알고리즘이 많은 시간이 걸리는지를 알아야 합니다.
  - 알고리즘을 분석하기 위해서는 이를 표시할 문법과 점근적 분석/표기의 어떤 형식이 필요합니다.
    - 최악의 경우(Worst case)
      - 알고리즘이 엄청난 시간을 소요하게 하는 입력을 정의합니다.
      - 입력은 알고리즘을 느리게 동작하게 하는 것으로 선택합니다.
    - 최선의 경우(Best case)
      - 알고리즘이 최단 시간을 소요하게 하는 입력을 정의합니다.
      - 입력은 알고리즘을 빠르게 동작하게 하는 것으로 선택합니다.
    - 평균의 경우(Average case)
      - 알고리즘의 실행 시간에 대한 예측을 제공합니다.
      - 입력은 무작위로 가정합니다.
  
### 2.9 점근적 표기(Asymptotic Notation)
  - 알고리즘을 최선, 평균 그리고 최악의 경우에 대한 식으로 표현할 때, 세 가지 경우 모두에 대한 상한계(상계, upper bounds)와 하한계(하계, lower bounds)를 정의해야 합니다.
